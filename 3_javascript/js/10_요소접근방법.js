...
function nameTest() {
  ...
  console.log(hobbyList); // NodeList = 정적/동적, forEach O | HTML = 동적, forEach X
  ...
  for(let i = 0; i < hobbyList.length; i++) {
    ...
    // input 요소.checked -> 요소가 체크되어 있으면 true, 아니면 false 반환
    // input 요소.checked = true  -> 해당 요소를 체크하겠다
    // input 요소.checked = false -> 해당 요소를 체크 해제하겠다

    if(hobbyList[i].checked) {
      str += hobbyList[i].value + " "; // 공백 없으면 띄어쓰기 안 돼서 나옴
      count++
    }
  }
...
// 올라온 키가 "Enter"키면 readValue() 함수를 호출
