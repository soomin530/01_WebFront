...
function executeFn2() {
  const start = Number(document.getElementById("inputNumber2-1").value); // 시작
  const end = Number(document.getElementById("inputNumber2-2").value); // 종료
  const val = Number(document.getElementById("inputNumber2-3").value); // 증감

  // 결과 출력할 ul 요소
  const ul = document.getElementById("result2"); // ul 태그
  ul.innerHTML = ""; // 이전 ul 사이에 작성된 내용을 모두 삭제
...
function executeFn3() {
  const input = Number(document.getElementById("input3").value); // iuput

  // 결과를 출력할 ul 요소
  const ul = document.getElementById("result3"); // ul 태그
  ul.innerHTML = ""; // 이전 내용 삭제

  // 입력받은 단이 2~9 사이가 아닐 경우
...
function check8() {
  ...
  for(let y = 1; y <= 4; y++) {
    ...
    for(let x = 1; x <= 5; x++) {
      str += x;
    // y 턴이 올 때마다 아래 반복 (x가 false가 될 때까지)
    // "1"  = "" + 1
    // "12" = "1" + 2
    // "123" = "12" + 3
    // "1234" = "123" + 4
...
function check17() {
  ...
  while(input !== null) { // 취소 누르기 전까지 무한 반복
    ...
    switch(input) {
      ...
      default : alert("메뉴에 작성된 번호만 선택해주세욥"); break;
