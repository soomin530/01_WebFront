...
console.log(1234); //숫자는 쌍따옴표 없이 작성
console.log("문자열은 양쪽에 쌍따옴표 작성");
console.log('JS는 홑따옴표도 문자열로 취급');
// console.log(문자열); -> 쌍따옴표 없이 쓰면 변수 취급
...
console.log("100" + 100); //"100100" - 텍스트가 숫자 형식으로 입력된 것 뿐, 쌍따옴표 안은 문자열로 인식

// 2) 괄호 내부에 , 를 작성해 각각의 값을 독립적으로 출력하기
// 형변환 없이 출력되어 문자열 따로, 다른 데이터타입 따로 출력됨
console.log("number1 : " , number1, "/ number2 :" , number2);
// "number1 :" 300 "/number" 400 - 같은 쌍따옴표에 들어온 것 X
...
// 같은 이름으로(변수명) 선언을 또다시 하려고 해서 에러 남
// let number3 = 500; // 빨간 줄 에러 남
...
// 중복 선언 불가
// 선언만 할 수 없음 (const PI; , const test; 이런 식으로)
// -> 선언 및 초기화가 동시에 이루어져야 함
...
// : let, const로 선언된 변수/상수는 변수가 선언된 블록(중괄호 {})
// 내부에서만 유효하다 라는 것을 의미한다.
...
console.log(foo1); // 123, 전역변수이기 때문에 (블록과 관계 없음) 접근 가능
console.log(foo2); // 456, 전역변수이기 때문에 (블록과 관계 없음) 접근 가능
...
// 아래는 함수의 선언만 이루어졌기 때문에 호출하지 않으면 콘솔창에 값 입력 X
...
example(); // 함수의 호출, 이렇게 호출을 해야 함수가 수행되면서 함수 내에 작성된 코드가 실행됨.
// 선언과 호출 세트.. 선언을 했으면 호출을 반드시 해라 !!!!!!!!!!!!!

// console.log(test3); // ReferenceError: test3 is not defined (참조할 수 없다는 에러)
...
// 변수의 선언과 초기화
var a = 10; // "=10"(초기화)은 호이스팅 되지 않음(선언만 호이스팅 하기 때문에)
// -> 초기화는 원래 코드의 위치에서 이루어짐
console.log(a); // 다음 라인에 작성하면 a 에 값(10)이 들어간 상태로 출력됨

// 2. let.const 의 호이스팅
// : let/const로 선언된 변수는 선언만 해당 스코프의 최상단으로 호이스팅됨.
// var 와 달리, let/const 로 선언된 변수는 초기화가 이루어지기 전에는 사용할 수 없음
...
// (초기화 되기 이전에 접근할 수 없다는 에러)

let x = 50; // 변수의 선언 및 초기화

console.log(x); // 50

// 이러한 특성들 때문에 let.const 는 var 보다 예측 가능한
// 코드를 작성할 수 있다.
...
// JS 배열은 [](대괄호)로 작성
const numbers = [10, 20, 30]; // numbers 라는 방 3개 생성, 각각 방을 index라고 부름
...
// console.log("numbers 배열 값 중 2번째 : ",numbers[2], typeof numbers[3] );
// -> undefined = numbers[3] 에는 아무 값도 할당되지 않아 undefined가 된다. (선언만 된 상태)


// 2) JS 객체
// 값을 K:V(Map) 형식으로 여러 개 저장하는 형태
// -> { K:V, K:V, K:V...} - {}(중괄호)안에 작성

// K(Key) : 값을 구분하는 이름(변수명과 비슷)
// V(value) : K에 대응되는 값 (변수에 대입되는 값과 비슷)

const user = { id : "user01", pw : "pass01", userName : "홍길동"};
// -> 상자 이름은 user / id, pw, userName 이 K / user01, pass01, 홍길동이 V
...
//                익명함수
// 변수명 == 함수명
//                             return : 반환한다 -> 나를 호출한 쪽으로
const sumFn = function(a, b) { return a + b ;}; 
// 함수를 정의할 때 사용(전달 받은)하는 변수 : 매개변수 (Parameter)
...
console.log( typeof sumFn ); // 함수 type: function
