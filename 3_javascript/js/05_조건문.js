...
function check4() {
  // 입력한 나이 값 얻어오기 (숫자 타입 형변환)
  const age = Number(inputAge.value);

  console.log(inputAge.value.length);
  // "문자열".length : 문자열의 길이

  // 입력된 나이의 길이가 0인 경우 == 미입력(입력 안 한 경우)
  if(inputAge.value.length == 0) {
    ...
    }else if(age <= 19) { // 청소년 14~19까지 (0~13세까지는 위에서 걸러짐)
      alert("청소년");
...
function calc(op) {
  ...
  switch(op) {
    ...
    // 맞는 case가 없을 경우에 적용할 기본값 (else의 역할)
